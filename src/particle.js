...
class TextParticle {
    ...
    draw(ctx, text, font) {
        if (this.isAlive) {
            ...
            ctx.font = font;
            ctx.fillStyle = this.color;
            ctx.fillText(text,this.x - ctx.measureText(this.text).width/2,this.y);
        }
    }
}
...
class TextParticleManager {
    ...
    addParticle(x, y, xDirection, yDirection, speed, lifespan,color) {
        const particle = new TextParticle(x, y, xDirection, yDirection, speed, lifespan,color);
        this.particles.push(particle);
    }
    ...
    drawParticles(ctx,text,font) {
        this.particles.forEach(particle => {
            particle.draw(ctx,text,font);
        });
    }
}